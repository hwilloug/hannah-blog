...
def lambda_handler(event, context):
    ...
    if reaction is None: 
        return {
            "statusCode": 400,
            "body": "'reaction' required in body",
            'headers' : {
    ...
    if comment_id is None: 
      return {
          "statusCode": 400,
          "body": "'comment_id' required in body",
          'headers' : {
              'Access-Control-Allow-Origin' : '*'
          }
      }
    ...
    sql = "INSERT INTO bookclub.comment_reactions (comment_id, reaction_id, username, reaction) VALUES (%s, %s, %s, %s)"

    try:
      cursor.execute(sql, (comment_id, reaction_id, username, reaction))
      conn.commit()
